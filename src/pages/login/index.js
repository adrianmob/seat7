...
import { getToken, getUserId, login, isAuthenticated } from "../../services/auth";
...
export default class Login extends Component {
	...
	hendleSignIn = async(e) => {
		e.preventDefault();
		const { password } = this.state;
		const email = this.state.email.toLowerCase();

		if (!email || !password) {
			this.setState({ error: "Fill in all data to login" });
		} else {
			try {
				document.querySelector('.form .btn-login').classList.add("load");
				const response = await api.post("/app_users/login", { email, password });
				console.log('response_user');
				console.log(response);
				login(response.data.id, response.data.userId, response.data.created, response.data.ttl);
				
				const token = getToken();
				const userId = getUserId();
				const response_user = await api.get(`app_users/${userId}?filter[include]=companies&access_token=${token}`);
				localStorage.setItem("user_type", response_user.data.user_type);

				setTimeout(() => {
					this.setState({ redirect: true })
				}, 1000);
			} catch (err) {
				document.querySelector('.form .btn-login').classList.remove("load");
				console.log(err);
				if(err == "Request failed with status code 401") {
					this.setState({ error: "Your account is suspended" });
				} else {
		...
		return (
			<div data-page="sign-in">
				...
				<div className="container-sign-in">
					...
					<form className="form" onSubmit={this.hendleSignIn}>
						{this.state.error &&
				...
				<Link to="/register" className="btn button-signin">Sign up</Link>
			</div>
		)
	};
}